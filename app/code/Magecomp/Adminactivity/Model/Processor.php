...
use \Magecomp\Adminactivity\Helper\Data;
use Magecomp\Adminactivity\Model\Config;
use Magento\Backend\Model\Auth\Session;
use Magecomp\Adminactivity\Model\Handler;
...
use Magecomp\Adminactivity\Model\Handler\Postdispatch;
...
class Processor
{
    ...
    const SKIP_MODULE_ACTIONS = [
        'mui_index_render',
        'adminactivity_adminactivity_index',
        'adminactivity_adminactivity_activitylogdetail',
        'adminactivity_adminactivity_revert',
        'adminactivity_loginaction_index',
    ];
    ...
    public function validate($model)
    {
        ...
        if ($this->eventConfig) {
            ...
            if (!empty($this->dataHelper->getConfigValue($pathConst))) {
    ...
    public function modelDeleteBefore($model)
    {
    ...
    public function modelEditAfter($model)
    {
        ...
        {
            $label = ($this->eventConfig['action'] == self::SAVE_ACTION) ? self::EDIT_ACTION : self::SAVE_ACTION;
        }
    ...
    public function addLog($activity, $logData, $model)
    {
        ...
        $this->activityLogs[] = [
            \Magecomp\Adminactivity\Model\Activity::class => $activity,
        ...
        try {
            if (!empty($this->activityLogs)) {
                foreach ($this->activityLogs as $model) {
                    $activity = $model[\Magecomp\Adminactivity\Model\Activity::class];
                    $activity->save();
                    $activityId = $activity->getId();

                    if (isset($model[\Magecomp\Adminactivity\Model\ActivityLog::class])) {
                        $logData = $model[\Magecomp\Adminactivity\Model\ActivityLog::class];
                        if ($logData) {
                    ...
                    if (isset($model[\Magecomp\Adminactivity\Model\ActivityLogDetail::class])) {
                        $detail = $model[\Magecomp\Adminactivity\Model\ActivityLogDetail::class];
    ...
    public function _initLog()
    {
        ...
        if(isset($this->eventConfig['module']))
            $activity->setModule($this->dataHelper->getActivityModuleName($this->eventConfig['module']));
        if(isset($this->eventConfig['action']))
    ...
    public function revertActivity($activityId)
    {
        try {
            ...
            if ($activityModel->getIsRevertable() === '0') {
                 ...
                 $this->messageManager->addNoticeMessage(__('Activity data has already been reverted'));
            } else {
                ...
                if ($activityModel->getId() && $this->activityRepository->revertActivity($activityModel)) {
                    ...
                    $result['error'] = false;
                    $this->dataHelper->markSuccess($activityId);
            ...
            $result['error'] = false;
            $this->dataHelper->markFail($activityId);
        }
    ...
    public function addPageVisitLog($module)
    {
        ...
        if ($this->dataHelper->isPageVisitEnable()
